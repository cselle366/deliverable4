...
<html lang="en">
<head>
<script src="https://kit.fontawesome.com/73fdd2eb2f.js" crossorigin="anonymous"></script>
...
<figure> 
<!-- makes collapasble names -->
    <input type="checkbox" id="athlete-{name.replace(' ', '-')}-toggle">
    
    <!-- Athlete's name as the clickable label -->
    <label for="athlete-{name.replace(' ', '-')}-toggle">{name}</label>
    
    <!-- Collapsible content that shows when the checkbox is checked -->
    <div class="collapsible-content">
        <figure>
            <img src="../images/profiles/{profile_pic}" data-lightbox="athlete-set" width="200" alt="Profile picture of {name}" class="thumbnail">
            <figcaption>{name}</figcaption>
        </figure>
        <dl>
            <dt>Place</dt><dd>{place}</dd>
            <dt>Time<i class="fa-solid fa-clock"></i></dt><dd>{time}</i></dd>
...
#ADDED
def generate_index_html(csv_files, output_folder):
    """
    Generate an index.html page that links to all meet HTML pages.
    """
    index_html_content = """
    <!DOCTYPE html>
    <html lang="en">
    ...
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Meet Results Index</title>
    <link rel="stylesheet" href="../css/reset.css">
    <link rel="stylesheet" href="../css/style.css">
    ...
    # Add a list item linking to each meet page
    for csv_file in csv_files:
        # Derive the HTML filename
        meet_name = os.path.splitext(csv_file)[0]
        html_filename = f"{meet_name}.html"
        index_html_content += f'<li><a href="{html_filename}">{meet_name.replace("_", " ").title()}</a></li>\n'
    ...
    <footer>
        ...
        <a href="https://sites.google.com/aaps.k12.mi.us/skylinecrosscountry2021/home">XC Skyline Page</a><br>
        Follow us on Instagram: <a href="https://www.instagram.com/a2skylinexc/" aria-label="Instagram">Instagram</a>
        </p>
    </footer>
    <script src="../dist/js/lightbox-plus-jquery.js"></script>
    <script src="../js/darkMode.js"></script> 
    ...
    with open(index_html_file, 'w', encoding='utf-8') as htmlfile:
        htmlfile.write(index_html_content)

    print(f"Index HTML file created successfully at '{index_html_file}'.")
######
def process_meet_files():
    ...
    #added
    generate_index_html(csv_files, meets_folder)
    #####
